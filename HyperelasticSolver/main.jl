...
# #############################################################################
# Если используется модуль (module), но не используется пакет (package), то включение
# модуля не тривиально. В частности, нельзя сделать <<using Strains>>,
...
#
# Этих проблем нет, если импортируется пакет, а не просто модуль.
#
...
# Делаем include на все модули, которые используются в main.jl
